...
JNIEXPORT void JNICALL Java_com_asdev_libjam_mt_TouchHandler_init(JNIEnv *env, jobject obj, jint dev){
	...
	update_method = (*env)->GetStaticMethodID(env, th_class, "onUpdate", "(DDI)V");	
...
void update(double x, double y, int id){
	/* called when touch update has happened 
	* this needs to call the java code to notify it
	*/
	(*lenv)->CallStaticVoidMethod(lenv, th_class, update_method, (jdouble) x,  (jdouble) y, (jint) id);
}
